#!/usr/bin/env bash
# Build the thunderbolt-tbrxe DKMS source package (tbframe.ko + tbrxe.ko in
# ONE deb) for Debian/Ubuntu. Companion to tools/ci/distro-package.sh: the
# package rides the tbfix version stream (dkms/dkms.conf PACKAGE_VERSION) and
# the produced deb depends on thunderbolt-tbfix-dkms (= version).
...
usage() {
	cat <<'USAGE'
Usage:
  tools/ci/distro-package-tbrxe.sh debian|ubuntu

Outputs thunderbolt-tbrxe-dkms_<version>_all.deb (+ .sha256) into OUT_DIR.
...
modname="thunderbolt-tbrxe"
pkgname="${modname}-dkms"
ibv_root="$repo_root/drivers/thunderbolt_ibverbs"
...
stage_source() {
	local stage="$1"
	local contaminant
	install -d -m 0755 "$stage/kernel" "$stage/proto"
	install -m 0644 "$repo_root/dkms/tbrxe/dkms.conf" "$stage/dkms.conf"
	sed -i "s/^PACKAGE_VERSION=.*/PACKAGE_VERSION=\"$version\"/" \
		"$stage/dkms.conf"
	install -m 0644 "$repo_root/dkms/tbrxe/Makefile" "$stage/Makefile"
	...
	# kernel, the installed tbfix header shim/symvers, and tbframe's own
	# fresh symvers.
	tar -C "$ibv_root/kernel" \
		--exclude='*.o' \
		--exclude='*.ko' \
		--exclude='*.mod' \
		--exclude='*.mod.c' \
	...
	# tbframe includes ../../proto/thunderbolt_negotiation.h. Bundle the ONE
	# canonical header (drivers/thunderbolt/) at that in-tree path -- a build
	# artifact copied from the canonical, not a second source.
	install -m 0644 "$repo_root/drivers/thunderbolt/thunderbolt_negotiation.h" \
		"$stage/proto/thunderbolt_negotiation.h"
...
build_deb() {
	...
	substitute "$repo_root/packaging/debian/control-tbrxe" "$stage/DEBIAN/control"
	substitute "$repo_root/packaging/debian/postinst-tbrxe" "$stage/DEBIAN/postinst"
	substitute "$repo_root/packaging/debian/prerm-tbrxe" "$stage/DEBIAN/prerm"
