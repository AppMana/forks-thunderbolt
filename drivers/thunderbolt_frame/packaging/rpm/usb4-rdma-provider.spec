...
Version:        @VERSION@
Release:        1%{?dist}
Summary:        tbrxe libibverbs userspace provider
...
thunderbolt_frame_rxe kernel devices. Install alongside the matching
...
install -m 0644 %{_sourcedir}/libtbrxe-rdmav*.so %{buildroot}%{provider_lib_dir}/
install -m 0644 %{_sourcedir}/tbrxe.driver %{buildroot}%{provider_etc_dir}/

%files
%{provider_lib_dir}/libtbrxe-rdmav*.so
%{provider_etc_dir}/tbrxe.driver
