#!/usr/bin/env bash
# Run the fork's KUnit suites (core thunderbolt negotiation, tbnet,
# thunderbolt_frame)
# for real via tools/testing/kunit/kunit.py against a v7.0.12 tree (the vendored
# subsystem's upstream base) with the fork's drivers overlaid. The fleet/host
# kernels lack CONFIG_KUNIT, so this stands up a
# throwaway kunit kernel (x86_64 qemu, because USB4/INFINIBAND need PCI -> not UML).
# The thunderbolt_frame_rxe engine suite has its own runner
# (rxe/tests/run-kunit.sh).
#
# Usage:
#   drivers/thunderbolt_frame/tools/run-kunit.sh [kunit.py filter args...]
#   KUNIT_TREE=/path/to/linux  drivers/thunderbolt_frame/tools/run-kunit.sh
#   KUNIT_LOCKDEP=1            drivers/thunderbolt_frame/tools/run-kunit.sh
#
# Examples:
...
rm -rf "$HDR_GEN"
# Drop wiring left in the tree by pre-rename runs (a stale
# 'source "drivers/thunderbolt_ibverbs/Kconfig"' with no directory behind it
# fails the Kconfig parse).
rm -rf "$TREE/drivers/thunderbolt_ibverbs"
sed -i '/thunderbolt_ibverbs/d' "$TREE/drivers/Kconfig" "$TREE/drivers/Makefile"
mkdir -p "$TREE/drivers/thunderbolt_frame"
rsync -a --delete \
	--exclude='*.o' --exclude='*.ko' --exclude='*.mod*' --exclude='.*.cmd' \
	--exclude='Module.symvers' --exclude='modules.order' \
	"$REPO/drivers/thunderbolt_frame/frame/" "$TREE/drivers/thunderbolt_frame/frame/"

# Single source of truth: derive the object + test lists for the
# thunderbolt_frame module from the repo's frame/Makefile (the DKMS build)
# so they never drift from this overlay.
TBF_MK="$REPO/drivers/thunderbolt_frame/frame/Makefile"
xformf() { tr ' \t\\' '\n\n\n' | sed -e '/^$/d' \
	-e 's#^tests/#frame/tests/#;t' -e 's#^#frame/#' \
	| tr '\n' ' '; }
TBF_Y=$(sed -n 's/^thunderbolt_frame-y *:= *//p' "$TBF_MK" | xformf)
TBF_K=$(sed -n 's/^thunderbolt_frame-\$(CONFIG_KUNIT) *+= *//p' "$TBF_MK" | xformf)

cat > "$TREE/drivers/thunderbolt_frame/Makefile" <<MK
# SPDX-License-Identifier: GPL-2.0  (generated by run-kunit.sh from frame/Makefile)
obj-\$(CONFIG_THUNDERBOLT_FRAME) += thunderbolt_frame.o
thunderbolt_frame-y := $TBF_Y
thunderbolt_frame-\$(CONFIG_KUNIT) += $TBF_K
ccflags-y := -I\$(src)/frame -I\$(src) -DDEBUG -Wno-unused-parameter
MK

cat > "$TREE/drivers/thunderbolt_frame/Kconfig" <<'KC'
# SPDX-License-Identifier: GPL-2.0  (generated by run-kunit.sh)
config THUNDERBOLT_FRAME
	tristate "Thunderbolt lossless frame service (KUnit)"
	depends on USB4 && INFINIBAND
KC

grep -q 'thunderbolt_frame/Kconfig' "$TREE/drivers/Kconfig" || \
	sed -i 's#source "drivers/thunderbolt/Kconfig"#source "drivers/thunderbolt/Kconfig"\nsource "drivers/thunderbolt_frame/Kconfig"#' "$TREE/drivers/Kconfig"
grep -q 'thunderbolt_frame/' "$TREE/drivers/Makefile" || \
	printf 'obj-$(CONFIG_THUNDERBOLT_FRAME) += thunderbolt_frame/\n' >> "$TREE/drivers/Makefile"
