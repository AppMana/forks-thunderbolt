...
## tbrxe.ko needs a provider that knows its driver id: 0004 or 0005

Patches 0001-0003 are about the *legacy* `usb4_rdma` engine (kernel
`ibdev.c`). Patches 0004 and 0005 are about `tbrxe.ko`, the rxe-derived
...
either. **So `tbrxe.ko` and an id-aware rdma-core provider must be rolled
