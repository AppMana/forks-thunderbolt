...
static int tbv_debugfs_summary_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "link_owner: %s\n",
		   tbv_link_owner_name(READ_ONCE(state->link_owner)));
	...
	seq_printf(s, "data_wr_no_capacity: %lld\n",
		   atomic64_read(&state->data_wr_no_capacity));
	...
	seq_printf(s, "data_rx_timeout_retry: %lld\n",
		   atomic64_read(&state->data_rx_timeout_retry));
