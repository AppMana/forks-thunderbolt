...
static void tb_scan_port(struct tb_port *port)
{
	 ...
	 * first. Mark it unplugged so the unregister funnel
	 * (tb_domain_unregister_unplugged_xdomains(), run after tb->lock
	 * is dropped) removes the device from the bus; tb_xdomain_remove()
